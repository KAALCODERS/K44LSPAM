...
DEAD_PIC = ALIVE_PIC if ALIVE_PIC else "https://telegra.ph/file/be526220ab16ee5504652.jpg"
  

DEADLY = "✯ 𝐊𝐀𝐀𝐋 𝐒𝐏𝐀𝐌 𝗛𝗲𝗿𝗲 ✯\n\n"
DEADLY += f"═══════════════════\n"
DEADLY += f"• **ᴘʏᴛʜᴏɴ ᴠᴇʀsɪᴏɴ** : `3.10.1`\n"
DEADLY += f"• **ᴛᴇʟᴇᴛʜᴏɴ ᴠᴇʀsɪᴏɴ** : `{version.__version__}`\n"
DEADLY += f"• **𝐊𝐀𝐀𝐋𝐒𝐏𝐀𝐌 ᴠᴇʀsɪᴏɴ**  : `{Kaalversion}`\n"
DEADLY += f"═══════════════════\n\n"   
...
async def alive(event):
  if event.sender_id in SUDO_USERS:
     await BOT0.send_file(event.chat_id,
                                  ...
                                  buttons=[
        [
        Button.url("𝐎𝐖𝐍𝐄𝐑", "https://t.me/garw_mishra"),
        Button.url("sᴜᴘᴘᴏʀᴛ", "https://t.me/kaal_riders")
        ],
        [
        Button.url("• ʀᴇᴘᴏ •", "https://github.com/garwmishra/K44LSPAM")
        ]
        ]
        )
...
async def tb(event):
    if event.sender_id == OWNER_ID:
        ...
        if sudousers:
            ...
            newsudo = f"{target}"
        await ok.edit(f"**Added `{target}` ** as a sudo user 🔱 Restarting 𝗞𝗔𝗔𝗟𝗦𝗣𝗔𝗠.. Please wait a minute...")
